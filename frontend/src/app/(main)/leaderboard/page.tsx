...
export default function LeaderboardPage() {
  ...
  return (
    <div>
      <div className="px-4 sm:px-6 py-5 flex items-center gap-2">
        <Trophy className="w-5 h-5 text-yellow-400" />
        <h1 className="text-[20px] font-extrabold text-text-primary">리더보드</h1>
      </div>

      {/* Table header */}
      <div className="flex items-center px-4 sm:px-6 py-2.5 text-[11px] sm:text-[12px] text-text-quaternary font-medium border-b border-border">
          ...
          {leaderboard?.map((entry) => {
            ...
            return (
              <div
                key={entry.userId}
                className={cn(
                  'flex items-center px-4 sm:px-6 py-3 sm:py-3.5',
                  isTop3 && 'bg-bg-secondary/30',
                )}
              >
                <div
                  className={cn(
                    'w-10 sm:w-12 text-center text-[13px] sm:text-[14px] font-bold shrink-0',
                ...
                <div className="flex items-center gap-2.5 sm:gap-3 flex-1 pl-2 min-w-0">
                  <div className="w-7 h-7 sm:w-8 sm:h-8 rounded-full bg-bg-tertiary flex items-center justify-center text-[10px] sm:text-[11px] font-bold text-text-secondary shrink-0">
                ...
                <span
                  className={cn(
                    'w-16 sm:w-24 text-right text-[13px] sm:text-[14px] font-semibold tabular-nums shrink-0',
