...
- **주문 시스템**: 시장가/지정가/STOP-LOSS/TAKE-PROFIT 주문, 부분 체결, 주문 취소 지원
- **매칭 엔진**: Price-Time Priority 알고리즘 기반의 주문 매칭
- **조건부 주문**: 목표가 도달 시 자동 체결 (트리거 기반)
- **포트폴리오**: 보유 자산 현황, 실현/미실현 손익(P&L) 계산, 거래 내역 조회, CSV 내보내기
- **관심종목**: 종목별 즐겨찾기 등록/해제, 대시보드 관심종목 탭

### 데이터 시각화
- **캔들스틱 차트**: 1분/5분/15분/1시간/1일 간격의 OHLCV 차트 (Lightweight Charts)
- **기술적 지표**: SMA(이동평균선), RSI(상대강도지수), 볼린저 밴드
- **호가창**: 실시간 매수/매도 주문 현황
- **Order Book 깊이 차트**: 매수/매도 누적 주문량 시각화
- **포트폴리오 분석**: 자산배분 파이차트, P&L 분석
- **리더보드**: 사용자 수익률 랭킹 (기간/정렬 필터, 메달 뱃지, 내 순위)
- **시장 지수 마키**: 24개 글로벌 시장 지수 실시간 표시
- **거래대금 Top 5**: 대시보드 상단 거래대금 상위 종목 표시

### 소셜 기능
- **실시간 채팅**: 1:1/그룹 채팅, 초대/퇴장, 읽음 확인, 메시지 검색
- **커뮤니티**: 전략 공유, 트레이더 프로필
- **온라인 상태**: 사용자 접속 상태 표시 (프레즌스)

### AI 분석
- **시장 분석 시그널**: 종목별 매매 추천 (STRONG_BUY ~ STRONG_SELL)
- **포트폴리오 AI 분석**: 분산 점수, 리스크 점수, 개선 제안
...
| Phase 3 | 11-13주 | 소셜/알림/고급 거래 (**완료**) | 채팅, 가격 알림, 알림 센터, 고급 주문, 기술적 지표, 커뮤니티, 접근성 |
| Phase 4 | 14-16주 | AI 기능 (**완료**) | AI 시장 분석 시그널, 포트폴리오 AI 분석 |
