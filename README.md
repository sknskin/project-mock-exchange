...
- **글로벌 시장 지수 마키**: Yahoo Finance v8 API 실제 데이터 (나스닥, S&P 500, KOSPI, VIX 등 18개 지수) + 5일 스파크라인 + 휠 관성 스크롤
- **감사 보고서 자동 감지**: 빌드 타임 매니페스트 생성으로 보고서 파일 추가 시 자동 반영
...
- [x] **Phase 3**: 소셜 기능 + 고급 알림 + 고급 거래
  ...
  - Admin 패널 확장 (시스템 설정, 서비스 헬스 모니터링, 감사 보고서 자동 감지, 사용자 상세)
  - 글로벌 시장 지수 마키 — Yahoo Finance v8 실제 데이터 (18개 지수, 5일 스파크라인, 휠 관성 스크롤)
