# VirtuEx 아키텍처 설계 요약 (한글)

## 전체 개요

VirtuEx는 실시간 모의 주식/암호화폐 거래 플랫폼으로, 마이크로서비스 아키텍처 기반의 프로덕션급 시스템입니다. 1,000명 이상의 동시 트레이더를 지원하며, 수평 확장이 가능하고, Kubernetes 환경에 배포할 수 있도록 설계되었습니다.
...
| Phase 3 | 11-13주 | 소셜/알림/고급 거래 (**완료**) | 채팅(메시지 검색), 가격 알림, 알림 센터, 온라인 상태, 통계, 고급 주문(STOP-LOSS/TAKE-PROFIT), 차트 기술적 지표(SMA/RSI/BB), 깊이 차트, 포트폴리오 분석, 커뮤니티, 접근성 개선 |
